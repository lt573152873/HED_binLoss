...
template <typename Dtype>
int SigmoidCrossEntropyLossLayer<Dtype>::comp (const void * a,const void *b)  
{  
...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_cpu(
  ...
  Dtype *loss_r = (Dtype *)malloc(sizeof(Dtype)*dim);
  for (int i = 0; i < dim; i++) {
	  loss_r[i] = input_data[i] * (target[i] - (input_data[i] >= 0)) - log(1 + exp(input_data[i] - 2 * input_data[i] * (input_data[i] >= 0)));
  }
  qsort(loss_r,dim,sizeof(Dtype),comp);
  free(loss_r);
}
