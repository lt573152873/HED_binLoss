...
/* modified loss function for fundus image vessel segmentation */
template <typename Dtype>
class SigmoidFineCrossEntropyLossLayer : public LossLayer<Dtype> {
 public:
  explicit SigmoidFineCrossEntropyLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param),
          sigmoid_layer_(new SigmoidLayer<Dtype>(param)),
          sigmoid_output_(new Blob<Dtype>()) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "SigmoidFineCrossEntropyLoss"; }
...
template <typename Dtype>
class BinLossLayer : public LossLayer<Dtype> {
 public:
  explicit BinLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param),
          sigmoid_layer_(new SigmoidLayer<Dtype>(param)),
          sigmoid_output_(new Blob<Dtype>()) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "BinLoss"; }
